...
def predict_close(stock_name, days_forward, start_date, train_end_date, end_date, plot_end_date):
    ...
    data_with_indicators['ROC'] = TA.ROC(data_with_indicators, period=days_forward)
    ...
    data_with_indicators[indicator_name] = data_with_indicators['Close'].shift(-days_forward)
    data_with_indicators = data_with_indicators.dropna()

    X_columns = ['ROC', 'KAMA', 'RSI', 'MOM']
    y_column = [indicator_name]
    model_data = data_with_indicators[start_date:train_end_date][X_columns + y_column]
    ...
    backtrader_data[predicted_indicator_name] = regr.predict(backtrader_data[X_columns])
    # Calc % price change based on price prediction
    backtrader_data['Predicted_Change_Pct'] = (backtrader_data[predicted_indicator_name]/backtrader_data[predicted_indicator_name].shift(days_forward)-1)

    # plot data with prediction
    #(backtrader_data[start_date:plot_end_date][[indicator_name, predicted_indicator_name]]).plot()
...
for stock_name in stocks:
    predict_close(stock_name, days_forward, start_date, train_end_date, end_date, plot_end_date)
...
class ModelMultiStrategy(bt.Strategy): 
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():    
                ...
                self.log('%s TRADE SIZE: %.0f' % (self.inmarketstock, self.size))
                self.log('%s BUY EXECUTED: %.2f' % (self.inmarketstock, order.executed.price) )        
            elif order.issell():             
                self.sellprice = order.executed.price
                self.inmarket = False
                self.log('%s SELL EXECUTED: %.2f' % (self.inmarketstock, order.executed.price))
    ...
    def next(self):
        ...
        if self.inmarket == False: 
            pred_changes = [self.datas[0].Predicted_Change_Pct[0], self.datas[1].Predicted_Change_Pct[0], self.datas[2].Predicted_Change_Pct[0]]
            ...
            if max(pred_changes) > 0:
                ...
                self.log('%s BUY CREATE: %.2f' % (self.inmarketstock, d.close[0]))
        ...
        # We are already in the market, look for a signal to CLOSE trades
            ...
            profit = 0
            if (self.buyprice != 0):
                profit = (self.datas[self.inmarketdataindex].close[0] - self.buyprice)/ self.buyprice
            # if profit % above tolerance, sell
            if (profit > self.params.profit_tolerance):
                self.log('%s CLOSE CREATE - PROFIT TOLERANCE: %.2f' % (self.inmarketstock, self.datas[self.inmarketdataindex].close[0]))
            ...
            # EXIT CONDITION #2: Always close after 3 days regardless of profit
                ...
                self.log('%s CLOSE CREATE - MAX DAYS: %.2f' % (self.inmarketstock, self.datas[self.inmarketdataindex].close[0]))
                self.order = self.sell(data = self.datas[self.inmarketdataindex])
