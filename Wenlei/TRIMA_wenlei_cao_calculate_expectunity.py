...
def convert_outputcsv_toDF (path):
    df= pd.read_csv(path, header=None) #r'C:\Users\Wenlei\Desktop\GQP\bt_output_regression.csv'
    ...
    df[['Entry price', 'Exit price' ]] = df[['Entry price', 'Exit price' ]].astype(float)
    return df
...
def calculateExpectunity(df,  NumnberofShare):
   # df.columns = ['Entry date', 'Entry price', 'Exit date', 'Exit price']
    ...
    avergeloss = df.loc[df['Profit/Loss']<0, ['Profit/Loss'] ].sum()/df.loc[df['Profit/Loss']<0, ['Profit/Loss'] ].count()
    df['Average Loss'] = pd.Series([abs(avergeloss) for x in range(len(df.index))])
    df['Largest Loss'] = pd.Series([abs(df.loc[df['Profit/Loss']<0, ['Profit/Loss'] ].min()) for x in range(len(df.index))])
...
print ("backtesting use default param value : TRIMA fast 7, slow 17. Expectunity %.2f "  % (combined_expectunity_calcuation(r'C:\Users\Wenlei\Desktop\GQP\exportfile\TRIMA_clf.csv', 100)))
